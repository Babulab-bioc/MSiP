...
#' @param gamma Paramter needed for all kernels except linear
...
#' defaults to 10
#' @param pr.plot Logcal value, indicating whether to plot precision-recall (PR) curve
#' @param roc.plot Logcal value, indicating whether to plot ROC curve
#' @return Data frame containing a classification result for all instances in the dataset
...
#' @description Classification using the Support Vector Machines (SVM) algorithm.
#' The function also computes the areas under the precision-recall (PR) and ROC curve to evalute the performance of the classifer.
#' @export
...
svmTrain  <-
    function(dtInput,impute = TRUE,p = 0.3,parameterTuning = TRUE,
        ...
        if(roc.plot) {
            ...
            rocP <- plot.roc(x_test$target,attr(pred.mod, "probabilities")[,2],
